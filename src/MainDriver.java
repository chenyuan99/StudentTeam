...
public class MainDriver {
    ...
    private static void createNewProfile(Scanner scan) {
        ...
        // low).
        long userID = Instant.now().toEpochMilli() + ((long)Math.random()
            % 10000);
        ...
        database.store(email, new StudentUser(userID, fName, lName, email,
            phone, major, year, password));
    ...
    private static void editProfile(Scanner scan) {
        ...
        int userSelection = NavigationMenu.displayEditProfile(scan,
            currentUser);
        scan.nextLine();
